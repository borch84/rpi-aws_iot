...
def getBaudrate(ser, fSize=None, checkModel=None):
    for baudrate in (2400, 4800, 9600, 19200, 38400, 57600, 115200):
        ...
        if b'comok' in r:
            ...
            noConnect = False
            cadena = r.decode("latin-1")
            #status, unknown1, model, fwversion, mcucode, serial, flashSize = r.strip("\xff\x00").split(',')
            status, unknown1, model, fwversion, mcucode, serial, flashSize = cadena.strip("\xff\x00").split(',')
...
def setDownloadBaudrate(ser, fSize, baudrate):
    ser.write(b"")
    cadena = "whmi-wri " + str(fSize) + "," + str(baudrate) + ",0"
    print (cadena)
    byte_object = cadena.encode("utf-8","strict")
    print (byte_object)
    #ser.write(b"whmi-wri " + str(fSize) + "," + str(baudrate) + ",0" + e)
